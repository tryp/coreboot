...
#include <cpu/intel/fsp_haswell/fsp_util.h>
#include "chip.h"
#include "haswell.h"
...
static void mc_add_dram_resources(device_t dev)
{
	...
	unsigned long index;
	unsigned long fsp_rsvd_mem_base;
	...
	/* 0xc0000 -> FSPB */
	fsp_rsvd_mem_base = (unsigned long) fsp_get_low_mem_top ();
	base_k = 0xc0000 >> 10;
	size_k = (unsigned long)(fsp_rsvd_mem_base >> 10) - base_k;
	ram_resource(dev, index++, base_k, size_k);

	/* FSPB -> TSEG */
	resource = new_resource(dev, index++);
	resource->base = fsp_rsvd_mem_base;
